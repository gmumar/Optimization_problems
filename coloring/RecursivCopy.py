...
def checkloc(colors,domain):
#Looks at colors and checks if its feasible
#If it is then prunes the domain and return -1
#Else return number of node causing the infeasibility
...
def setValueloc(colors,node,domain):
    global lvlMax
    global adj
    ...
    for i in range(maxChoice+1):
        if(len(domain[node])>i):
            colors[node] = domain[node][i]
        else:
            for l in range(len(domain[node])):
                for a in adj[node]:
                    if(l == colors[int(a)]):
                        pass
                    else:
...
def solve(colors,node,domain):
    ...
    setValueloc(colors,node,domain)
    
    locDom = copy.deepcopy(domain)

    
    if(checkloc(colors,domain)==-1):
        #Feasible
        #Maintain local copy of domain
        pass
    else:
        #infeasible
        domain = copy.deepcopy(locDom)
        
    solve(colors,node+1,domain)
